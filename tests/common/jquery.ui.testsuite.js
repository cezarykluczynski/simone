...
// Window has it own compatibility list
// QUnit.config.urlConfig.push({
// 	id: "jquery",
...
function getWidget( widget ) {
	return widget === "window" ? $.simone.window : $.ui[ widget ];
}

function testWidgetDefaults( widget, defaults ) {
	var pluginDefaults = getWidget( widget ).prototype.options;
	...
	if ( $.uiBackCompat === false ) {
		test( "$.widget overrides", function() {
			...
			$.each([
				...
				strictEqual( getWidget( widget ).prototype[ method ],
...
function testBasicUsage( widget ) {
	test( "basic usage", function() {
		...
		var defaultElement = getWidget( widget ).prototype.defaultElement;
...
TestHelpers.commonWidgetTests = function( widget, settings ) {
	...
	// ignore defaults test - it just tests values from prototype
	// against values copied from the prototype at some point
	// testWidgetDefaults( widget, settings.defaults );
	testWidgetOverrides( widget );
	testBasicUsage( widget );
	test( "version", function() {
		expect( 1 );
		ok( "version" in getWidget( widget ).prototype, "version property exists" );
	});
};
